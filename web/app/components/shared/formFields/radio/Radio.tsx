...
import { RadioGroup, Radio as RadioField, FormControlLabel, FormControl, FormLabel } from '@mui/material'
...
const Radio: FC<IRadioProps> = memo(({options, uid, setParam, getParam}) => {
  ...
  return (
    <FormControl>
      ...
      >
        {options.map(option => (
          <FormControlLabel value={option.value} control={<RadioField />} label={option.text} />
        ))}
      </RadioGroup>
    </FormControl>
  )
